...
const FilePreview = React.memo(({ file, onLoadError }) => {
  ...
  const loadPreview = useCallback(async () => {
    if (!file?.id || state.previewUrl) return;
    ...
    try {
      ...
      const previewUrl = await previewQueue.add(
        file.id,
        loadAndProcessFile,
        priority
      );

      if (isMounted.current && previewUrl) {
        setState({
          isLoading: false,
          error: false,
          previewUrl
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting && !state.previewUrl && !state.error) {
            loadPreview();
          }
        });
      },
  ...
  }, [loadPreview, state.previewUrl, state.error]);

  // Initial load effect for images
  useEffect(() => {
    const fileExt = file?.name?.split('.').pop().toLowerCase();
    if (['png', 'jpg', 'jpeg', 'gif'].includes(fileExt)) {
      loadPreview();
  ...
  }, [file?.name, loadPreview, state.previewUrl]);

  if (state.error) {
    return (
      <PreviewContainer>
        <PreviewError onRetry={() => {
